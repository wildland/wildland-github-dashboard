...
module.exports = {
	main: function (request, reply) {
		if (request.auth.isAuthenticated) {
		    ...
		    return ghorg.member(request.auth.credentials.profile.username, function (err, isMember, headers) {
		    	if (err) { throw err; }
				if (isMember) {
					return client.get('/orgs/'+ org + '/issues', {}, function (err, status, body, headers) {
						if (err) { throw err; }
